import type { Flac, ReadyEvent } from "../index.d";
...
export class BeforeReadyHandler<Target, ChacheableCalls> {
    ...
    public handleBeforeReady(
        funcName: ChacheableCalls,
        args: ArrayLike<any>
    ): boolean {
            ...
            if (!this._isWaitOnReady) {
                ...
                this._onReadyHandler = (_evt: ReadyEvent): void => {
                    if (this._beforeReadyCache) {
                        this._beforeReadyCache.forEach((entry) =>
                            (this._target as any)[entry.func].apply(
                                this._target,
                                entry.args
                            )
                        );
