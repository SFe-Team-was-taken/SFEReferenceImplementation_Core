...
export class Decoder {
    ...
    constructor(
        ...
        this._onRead = (
            bufferSize: number
        ): ReadResult | CompletedReadResult => {
            if (this._onReadData) {
        ...
        this._onError = (code, description) => {
            ...
            console.error(
                `Decoder[${this._id}] encoutered error (${code}): ${description}`
            );
        };

        if (this._options?.autoOnReady) {
            this._beforeReadyHandler = new BeforeReadyHandler<
                Decoder,
                ChacheableDecoderCalls
            >(this, true, this.Flac);
        }
    ...
    private _init(isDecodeOgg?: boolean): void {
        if (this._id) {
            const state = isDecodeOgg
                ? this.Flac.init_decoder_ogg_stream(
                      this._id,
                      this._onRead,
                      this._onWrite,
                      this._onError,
    ...
    public reset(options?: DecoderOptions): boolean {
        if (this._id && !!this.Flac.FLAC__stream_decoder_reset(this._id)) {
            if (options) {
                ...
                const isVerify =
                    this.Flac.FLAC__stream_decoder_get_md5_checking(this._id);
                if (
                    typeof this._options.verify !== "undefined" &&
                    isVerify != /*non-exact comparision*/ this._options.verify
                ) {
                    this.Flac.FLAC__stream_decoder_set_md5_checking(
                        this._id,
    ...
    public decode(flacData: Uint8Array<ArrayBuffer>): boolean {
        if (this._id && this._isInitialized && !this._isFinished) {
            ...
            if (
                !!this.Flac.FLAC__stream_decoder_process_until_end_of_stream(
                    this._id
                )
            ) {
                return this._finish();
     ...
     */
    public decodeChunk(
        flacData?: Uint8Array<ArrayBuffer>,
        flushCache?: boolean
    ): boolean {
        if (this._id && this._isInitialized && !this._isFinished) {
            if (!this._onReadData) {
    ...
    private _createReadChunkFunc(): decoder_read_callback_fn {
        ...
        return (bufferSize: number): ReadResult | CompletedReadResult => {
            ...
            const end =
                start === size ? -1 : Math.min(start + bufferSize, size);
            ...
            if (numberOfReadBytes < bufferSize) {
                ...
                const nextSize =
                    this._inputCache.length > 0
                        ? this._inputCache[0].buffer.byteLength
                        : 0;
                this._currentInputCacheOffset = 0;
    ...
    private _handleBeforeReady(
        funcName: ChacheableDecoderCalls,
        args: ArrayLike<any>
    ): boolean {
