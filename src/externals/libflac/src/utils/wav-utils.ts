 ...
 */
export function wav_file_processing_convert_to32bitdata(
    arraybuffer: ArrayBuffer,
    bps: number
): Int32Array<ArrayBuffer> | undefined {
    var decFunc;
    switch (bps) {
        case 8:
...
function convert_24bitdata_to32bitdata(dataView: DataView, i: number): number {
    var b =
        (((dataView.getUint8(i + 2) << 8) | dataView.getUint8(i + 1)) << 8) |
        dataView.getUint8(i);
