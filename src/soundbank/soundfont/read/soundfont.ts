...
export class SoundFont2 extends BasicSoundBank {
    ...
    public constructor(arrayBuffer: ArrayBuffer, sfe: boolean) {
        ...
        if (isfeChunk !== undefined) {
            ...
            if (
                sftyStr === "SFE standard" ||
                sftyStr === "SFe standard" ||
                sftyStr === "SFE containerised"
            ) {
                ...
                SpessaLog.groupEnd();
                this.parsingError(`Invalid SFe bank type: "${sftyStr}"`);
            }

            if (isfeChunks.sfvx?.data.length === 46) {
                const sfeMajVer = readLittleEndianIndexed(
                    isfeChunks.sfvx?.data,
                    2
                );
                const sfeMinVer = readLittleEndianIndexed(
                    isfeChunks.sfvx?.data,
                    2
                );
                const sfeSpecType = readBinaryStringIndexed(
                    isfeChunks.sfvx?.data,
                    20
                );
                const sfeDraft = readLittleEndianIndexed(
                    isfeChunks.sfvx?.data,
                    2
                );
                const sfeVerStr = readBinaryStringIndexed(
                    isfeChunks.sfvx?.data,
                    20
                );
                ...
                if (sfeMajVer >= 5) {
                    ...
                    if (sfeSpecType === "Draft") {
                        SpessaLog.info(
                            "%cThis bank is written to a SFe draft specification.",
                            ConsoleColors.warn
                        );
                        SpessaLog.info(
            ...
            if (isfeChunks.flag?.data.length % 6 === 0) {
                while (
                    isfeChunks.flag?.data.length >
                    isfeChunks.flag?.data.currentIndex
                ) {
                    this.loadSupportedFlags(supportedFlags);
                    sfeFlags.push({
                        branch: readLittleEndianIndexed(
                            isfeChunks.flag?.data,
                            1
                        ),
        ...
        if (
            ...
            SpessaLog.info(
                "%cThe sm24 chunk is supported. Verifying sm24 chunk...",
                ConsoleColors.warn
            );
        ...
        const samples =
            xdtaChunk &&
            xChunks.shdr &&
            shdrChunk.data.length === xChunks.shdr?.data.length
                ? readSamples(
                      shdrChunk,
                      sampleData,
                      xdtaChunk === undefined,
        ...
        const instruments =
            xdtaChunk &&
            xChunks.inst &&
            instChunk.data.length === xChunks.inst?.data.length
                ? readInstruments(instChunk, true, xChunks.inst)
                : readInstruments(instChunk, false, undefined);
        ...
        const presets =
            xdtaChunk &&
            xChunks.phdr &&
            phdrChunk.data.length === xChunks.phdr?.data.length
                ? readPresets(phdrChunk, this, true, xChunks.phdr)
                : readPresets(phdrChunk, this, false, undefined);
    ...
    protected verifySFeHeader(chunk: RIFFChunk) {
        if (
            chunk.header.toLowerCase() !== "riff" &&
            chunk.header.toLowerCase() !== "rifs"
        ) {
            SpessaLog.groupEnd();
            this.parsingError(
    ...
    protected verifyFlag(
        supported: FeatureFlagList,
        bankFlags: SFeFeatureFlag
    ) {
        if (
            !(
                supported.featureName === "Reserved" ||
                supported.featureName === "End of Flags"
            )
        ) {
